...
const EditPropertyScreen = () => {
  ...
  const validateForm = () => {
    ...
    else if (isNaN(Number(formData.area)) || Number(formData.area) <= 0) newErrors.area = 'Area must be a positive number';
    if (!formData.rooms.trim()) newErrors.rooms = 'Number of rooms is required';
    else if (isNaN(Number(formData.rooms)) || Number(formData.rooms) <= 0) newErrors.rooms = 'Number of rooms must be a positive number';
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await dispatch(updateProperty({ propertyId: property.id, updateData }) as any);
      ...
      >
        ...
        >
          <View style={styles.header}>
            ...
            <Text style={styles.subtitle}>Update your property listing</Text>
          </View>
